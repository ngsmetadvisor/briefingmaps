...
LLJ_COLORS = ['#888888', '#C5E7FF', '#6B8BFF', '#cc99ff', '#9900cc', '#ff00cc']
...
m.get_root().html.add_child(Element(
    ...
    '      if(layer.options && layer.options.pane==="landPane") return;\n'
    '      if(layer.setStyle && layer.eachLayer){\n'
    '        layer.eachLayer(function(sub){\n'
    '          if(sub.options && sub.options.pane==="albertaPane") return;\n'
    '          if(sub.options && sub.options.pane==="landPane") return;\n'
    '          if(sub.setStyle){\n'
...
if (_synLevel === "850" || _synLevel === "500") {{
    ...
    if (_synLevel === "850" && _synShowLLJ) {{
      var lljSwatches =
        ...
        + _synSwatchCol("20kt", "#C5E7FF")
        + _synSwatchCol("25kt", "#6B8BFF")
  ...
  if (_synLevel === "850" && _synShowLLJ) {{
    ...
    if (_lljFills.length) {{
      ...
      _lljFills.forEach(function(poly) {{
        ...
        L.polygon([poly.coords], {{
          color: "none", weight: 0,
          fillColor: poly.color, fillOpacity: _LLJ_FILL_OPACITY,
...
def _extract_wind_barb_pts(sub_df, spd_col, dir_col, size=24, spacing_km=75.0):
    pts = []
    _wsub = sub_df.dropna(subset=[spd_col, dir_col])
    if _wsub.empty:
        return pts

    # Decimate to a coarse lat/lon grid before generating barbs — without this,
    # every raw 0.5° extraction-grid point (tens of thousands per timestep)
    # gets its own SVG built, which is the dominant cost in this pipeline.
    _dlat = spacing_km / 111.0
    _lats_arr = _wsub['lat'].values.astype(float)
    _lons_arr = _wsub['lon'].values.astype(float)
    _cell_lat = np.round(_lats_arr / _dlat).astype(int)
    # longitude spacing narrows with latitude — use a per-row cos(lat) correction
    _dlon = spacing_km / (111.0 * np.maximum(0.15, np.cos(np.radians(_lats_arr))))
    _cell_lon = np.round(_lons_arr / _dlon).astype(int)
    _cell_key = _cell_lat.astype(np.int64) * 100000 + _cell_lon.astype(np.int64)
    _, _first_idx = np.unique(_cell_key, return_index=True)
    _wsub = _wsub.iloc[np.sort(_first_idx)]
...
#gem-time-slider{width:320px;accent-color:#4a7fc1;cursor:pointer;}
#gem-ts-label{color:#2a3a6a;font-size:9px;min-width:200px;font-weight:bold;}
</style>
<div id="gem-bar">
   <div class="bar-section">
    <span class="bar-label">Export</span>
