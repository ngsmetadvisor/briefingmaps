...
Original file is located at
    https://colab.research.google.com/drive/1sNMk9iQvKph1U1w5IK20og6S9Di_9xzy
"""
...
print(f'Output directory: {os.path.abspath(_OUTPUT_DIR)}')
...
_svg_out_path = os.path.join(_OUTPUT_DIR, 'station_model_examples.svg')
...
plt.close(_fig_demo)
_buf.seek(0)
_png_path = os.path.join(_OUTPUT_DIR, 'temp_band_legend.png')
...
for (date_val, hr), _grp_llj in ua_summary_df.groupby(
    ...
    # For each forced station, find nearest RDPS grid point
    _stn_rows_llj = []
...
# Build the LLJ Prog folium map — same base map/borders/contours as synoptic,
# but injects the LLJ-only station JSON
...
# Inject JS with LLJ station data replacing the standard UA station data
_llj_js = _js.replace(
...
_output_files = [
    ...
    ('llj_prog.html',              '🌬️ LLJ Prog',                 '850 hPa LLJ station network — fixed 18-station Alberta grid, heights, isotherms & wind barbs'),
