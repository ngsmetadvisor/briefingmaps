...
GEM_GRID_DEG =   2.0    # grid spacing in degrees
...
print('500 & 700 Ridge and Trough')
import pandas as pd
...
print('Building synthetic metar_records from GEM surface data...')
...
for _, r in _sfc.iterrows():
    ...
    metar_records.append({
        ...
        # precipitation — 12h accumulation from GEM QPF12H
        'precip_accum': float(r['QPF12H']) if pd.notna(r.get('QPF12H')) else None,

        # met fields — not available from GEM surface rows, set to safe defaults
        'temp':            None,
        'dew':             None,       # was 'dwpt' — fixed
        'wind_dir':        None,       # was 'wdir' — fixed
        'wind_spd':        None,       # was 'wspd' — fixed
        'wind_gust':       None,       # was missing
        'flt_cat':         'VFR',      # was missing — default avoids KeyError
        'rh':              None,       # was missing
        'vis':             None,       # was missing
        'clouds':          [],         # was 'sky' — fixed
        'weather':         '',         # was 'wx' — fixed
        'tendency':        None,       # was missing
        'pressure_change': None,       # was missing
...
metar_records = sorted(
    metar_records,
    key=lambda x: (x['time'], x['icao'])
)
...
print('GEM Surface Smoothing controls')
print(f'  MSLP rbf={MSLP_RBF_SMOOTHING}, σ={MSLP_SIGMA}, interval={MSLP_INTERVAL}')
print(f'  QPF12H rbf={QPF_RBF_SMOOTHING}, σ={QPF_SIGMA}, interval={QPF_INTERVAL}')
...
print(f'  MSLP valid   : {_sfc_df["MSLP"].notna().sum()}')
print(f'  QPF12H valid : {_sfc_df["QPF12H"].notna().sum()}')

if _sfc_df.empty:
    raise RuntimeError('❌ No surface rows (PRES==0) in ua_raw_df')
...
for (_date, _hr) in _sfc_times:
    ...
    print(f'  {len(_sub)} GEM surface points')

    # ── MSLP GRID ───────────────────────────────────────────────────────────
    slp_grid, lon_vec, lat_vec, obs_lons, obs_lats = _gem_build_grid(
        _sub, 'MSLP',
        rbf_smoothing=MSLP_RBF_SMOOTHING,
        sigma=MSLP_SIGMA
    )

    if slp_grid is not None:
        print(f'  ✓ MSLP {slp_grid.shape} {slp_grid.min():.1f}–{slp_grid.max():.1f} hPa')

    # ── QPF GRID (reuses MSLP lon_vec/lat_vec) ───────────────────────────────
    qpf_grid, _, _, _, _ = _gem_build_grid(
        _sub, 'QPF12H',
        rbf_smoothing=QPF_RBF_SMOOTHING,
        sigma=QPF_SIGMA,
        lon_vec=lon_vec,
        lat_vec=lat_vec
    )
    ...
    if qpf_grid is not None:
        ...
        print(f'  ✓ QPF12H {qpf_grid.shape} {qpf_grid.min():.1f}–{qpf_grid.max():.1f} mm')

    _mslp_segs = _count_contours(slp_grid, lon_vec, lat_vec, MSLP_INTERVAL)
    _qpf_segs  = _count_contours(qpf_grid, lon_vec, lat_vec, QPF_INTERVAL)
...
# expose last timestep as bare globals for Block 03 compatibility
if slp_grid is not None:
...
for _k in _ts_ua:
    ...
    for _lvl in ['850', '500']:
        ...
        for f in fills:
            # Remove empty hole arrays to save space
            if 'holes' in f and not f['holes']:
                del f['holes']
...
# @title
# ── Cell UA-2d. GEM Surface map — MSLP contours + QPF fill, time slider ──────
...
for _key in _sfc_keys:
    ...
    _qpf_bands     = _extract_qpf_bands(_qpf, _latv, _lonv) if _qpf is not None else []
      ...
      L.marker([c.lat,c.lon],{{
        icon:L.divIcon({{
          html:'<div style="font-size:52px;font-weight:bold;'
              ...
              +c.type+'<br>'
              +'<span style="font-size:16px;font-weight:normal;font-family:Courier New,monospace;">'
              +Math.round(c.val)+'</span></div>',
